...
/* Same "Spec Tag" identity as the wide Settings view — see
   SettingsWide.tsx for the full design note. This is the contained
   standalone rendering (e.g. routed directly at /settings). */
...
function TagPunch() {
  return (
    <svg width="34" height="34" viewBox="0 0 34 34" className="flex-shrink-0" aria-hidden="true">
      ...
      <path d="M 13.5 10 Q 5 20 13 27 Q 20 33 17 27" fill="none" stroke={PIN} strokeWidth="1.3" strokeDasharray="2 2.4" strokeLinecap="round" style={{ animation: 'threadDraw 1.1s 0.15s ease-out both' }} />
    </svg>
  );
}
...
export function AdminSettingsView() {
  ...
  return (
    <div className="max-w-5xl space-y-6" style={{ fontFamily: "'IBM Plex Sans', sans-serif", color: INK }}>
      <style>{FONT_IMPORT}</style>

      <div className="tag-in">
        <MonoLabel>Shop settings — spec sheet</MonoLabel>
        <h1 className="mt-1 text-2xl sm:text-3xl italic leading-tight" style={{ fontFamily: "'Fraunces', serif", fontWeight: 600, color: INK }}>
          Every detail, pinned in place.
        </h1>
        <p className="mt-2 text-sm max-w-xl" style={{ color: MUTED }}>Manage shop information, order rules, and notification preferences.</p>
      </div>
      ...
      <button onClick={save} className="tag-in inline-flex items-center gap-2 px-5 py-3 text-[10px] font-medium uppercase tracking-[0.15em] transition-colors" style={{ animationDelay: '0.25s', background: PIN, color: '#F4F7F1' }} onMouseEnter={(e) => { e.currentTarget.style.background = '#3E5A46'; }} onMouseLeave={(e) => { e.currentTarget.style.background = PIN; }}>
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        className="w-full bg-transparent px-0.5 py-2 text-[14px] outline-none border-0 border-b transition-colors"
...
function SwatchToggle({ label, detail, checked, onChange }: { label: string; detail: string; checked: boolean; onChange: (checked: boolean) => void }) {
  return (
    <div className="flex items-center justify-between gap-6 py-3 border-b" style={{ borderColor: LINE }}>
      <div>
        <span className="block text-sm font-medium" style={{ color: INK }}>{label}</span>
        <span className="mt-0.5 block text-xs" style={{ color: MUTED }}>{detail}</span>
      </div>
      <div className="flex-shrink-0 flex border" style={{ borderColor: checked ? PIN : LINE }}>
        <button type="button" onClick={() => onChange(false)} aria-pressed={!checked} className="px-3 py-1.5 text-[10px] uppercase tracking-[0.1em] transition-colors" style={!checked ? { background: MUTED, color: PAPER } : { background: PAPER, color: MUTED }}>Off</button>
        <button type="button" onClick={() => onChange(true)} aria-pressed={checked} className="px-3 py-1.5 text-[10px] uppercase tracking-[0.1em] transition-colors" style={checked ? { background: PIN, color: '#F4F7F1' } : { background: PAPER, color: MUTED }}>On</button>
      </div>
    </div>
  );
}
